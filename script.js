...
function normalizePublicPath(pathname = "/") {
  ...
  if (!rawPath || rawPath === "/" || lowerPath === "/index.html" || lowerPath.endsWith("/index.html")) {
    return "/draws";
  }
...
function getPlayerRouteSlug(playerOrId, tournament = appStore.tournament) {
  const player = typeof playerOrId === "string"
    ? tournament.lookup.playerById[playerOrId]
    : playerOrId;

  return player ? slugifyRouteSegment(player.name || player.id) : "";
}

function getEditionRouteSlug(editionOrId, tournament = appStore.tournament) {
  const edition = typeof editionOrId === "string"
    ? getEditionById(tournament, editionOrId)
    : editionOrId;

  return edition ? slugifyRouteSegment(edition.name || edition.id) : "";
}

function getPlayerByRouteSlug(slug, tournament = appStore.tournament) {
  const normalizedSlug = slugifyRouteSegment(slug);

  return tournament.players.find((player) => getPlayerRouteSlug(player, tournament) === normalizedSlug) ?? null;
}

function getEditionByRouteSlug(slug, tournament = appStore.tournament) {
  const normalizedSlug = slugifyRouteSegment(slug);

  return tournament.editions.find((edition) => getEditionRouteSlug(edition, tournament) === normalizedSlug) ?? null;
}

function getMatchRouteId(matchOrId) {
  const matchId = typeof matchOrId === "string" ? matchOrId : matchOrId?.id ?? "";

  return encodeURIComponent(String(matchId).trim());
}

function getHistoryRouteBase(editionId = appStore.ui.activeHistoryEditionId, tournament = appStore.tournament) {
  const edition = editionId ? getEditionById(tournament, editionId) : null;

  return edition ? `/historia/${getEditionRouteSlug(edition, tournament)}` : "/historia";
}

function getRouteForUiState(ui = appStore.ui, tournament = appStore.tournament) {
  if (ui.activePublicView === "players") {
    const activePlayer = ui.activePlayerDetailId
      ? tournament.lookup.playerById[ui.activePlayerDetailId]
      : null;

    return activePlayer
      ? `/jugadores/${getPlayerRouteSlug(activePlayer, tournament)}`
      : "/jugadores";
  }

  if (ui.activePublicView === "history") {
    const activeEdition = ui.activeHistoryEditionId
      ? getEditionById(tournament, ui.activeHistoryEditionId)
      : null;
    ...
    if (ui.activeMatchDetailId) {
      const activeMatch = getMatchById(tournament, ui.activeMatchDetailId);

      if (activeMatch && activeMatch.editionId === activeEdition.id) {
        return `${getHistoryRouteBase(activeEdition.id, tournament)}/partido/${getMatchRouteId(activeMatch)}`;
      }
    }

    return getHistoryRouteBase(activeEdition.id, tournament);
  }

  if (ui.activeMatchDetailId) {
    const activeMatch = getMatchById(tournament, ui.activeMatchDetailId);

    if (activeMatch && activeMatch.editionId === tournament.currentEditionId) {
      return `/draws/partido/${getMatchRouteId(activeMatch)}`;
    }
  }
...
function parsePublicRoute(pathname, tournament = appStore.tournament) {
  const normalizedPath = normalizePublicPath(pathname);
  const segments = normalizedPath
    .split("/")
    .filter(Boolean)
    .map(decodeRouteSegment);
  const rootSegment = segments[0]?.toLowerCase() ?? "draws";

  if (rootSegment === "draws") {
    ...
    if (segments.length === 3 && segments[1]?.toLowerCase() === "partido") {
      const match = getMatchById(tournament, segments[2]);

      if (match && match.editionId === tournament.currentEditionId) {
        return {
          canonicalPath: `/draws/partido/${getMatchRouteId(match)}`,
          ui: {
            activePublicView: "draws",
            activeMatchDetailId: match.id,
            activePlayerDetailId: null,
            activeHistoryEditionId: null,
          },
        };
  ...
  if (rootSegment === "jugadores") {
    if (segments.length === 1) {
      return {
        canonicalPath: "/jugadores",
        ui: {
          activePublicView: "players",
          activeMatchDetailId: null,
          activePlayerDetailId: null,
          activeHistoryEditionId: null,
        },
    ...
    if (segments.length === 2) {
      const player = getPlayerByRouteSlug(segments[1], tournament);

      if (player) {
        return {
          canonicalPath: `/jugadores/${getPlayerRouteSlug(player, tournament)}`,
          ui: {
            activePublicView: "players",
            activeMatchDetailId: null,
            activePlayerDetailId: player.id,
            activeHistoryEditionId: null,
          },
        };
      }
  ...
  if (rootSegment === "historia") {
    if (segments.length === 1) {
      return {
        canonicalPath: "/historia",
        ui: {
          activePublicView: "history",
          activeMatchDetailId: null,
          activePlayerDetailId: null,
          activeHistoryEditionId: null,
        },
    ...
    const edition = getEditionByRouteSlug(segments[1], tournament);
    ...
    if (segments.length === 2) {
      return {
        canonicalPath: `/historia/${getEditionRouteSlug(edition, tournament)}`,
        ui: {
          activePublicView: "history",
          activeMatchDetailId: null,
          activePlayerDetailId: null,
          activeHistoryEditionId: edition.id,
        },
      };
    }

    if (segments.length === 4 && segments[2]?.toLowerCase() === "partido") {
      const match = getMatchById(tournament, segments[3]);

      if (match && match.editionId === edition.id) {
        return {
          canonicalPath: `/historia/${getEditionRouteSlug(edition, tournament)}/partido/${getMatchRouteId(match)}`,
          ui: {
            activePublicView: "history",
            activeMatchDetailId: match.id,
            activePlayerDetailId: null,
...
function applyPublicUiState(uiState) {
  appStore.ui.activePublicView = uiState.activePublicView;
  appStore.ui.activeMatchDetailId = uiState.activeMatchDetailId;
  appStore.ui.activePlayerDetailId = uiState.activePlayerDetailId;
  appStore.ui.activeHistoryEditionId = uiState.activeHistoryEditionId;
}

function navigateToPublicPath(pathname, options = {}) {
  const route = parsePublicRoute(pathname, appStore.tournament);

  if (!route) {
    return;
  }

  applyPublicUiState(route.ui);
  ...
  if (typeof window !== "undefined") {
    ...
    if (route.canonicalPath !== currentPath) {
      window.history[options.replace ? "replaceState" : "pushState"](statePayload, "", route.canonicalPath);
    } else if (options.replace || !window.history.state?.__ramosOpen || window.history.state?.path !== route.canonicalPath) {
      window.history.replaceState(statePayload, "", route.canonicalPath);
    }
  }
...
function syncPublicRouteFromLocation() {
  const route = parsePublicRoute(
    typeof window !== "undefined" ? window.location.pathname : "/draws",
    appStore.tournament,
  );
  ...
  if (typeof window !== "undefined") {
    ...
    if (route.canonicalPath !== currentPath || !window.history.state?.__ramosOpen || window.history.state?.path !== route.canonicalPath) {
      window.history.replaceState(statePayload, "", route.canonicalPath);
    }
  }
    ...
    <div class="match-detail-overlay">
      ...
      >
        <div class="match-detail-header">
          ...
          <button class="match-detail-close" type="button" data-match-detail-close aria-label="Cerrar detalle del partido">
            &times;
          </button>
        </div>
    ...
    <div class="match-detail-overlay">
      ...
      >
        <div class="match-detail-header">
          ...
          <button class="match-detail-close" type="button" data-player-detail-close aria-label="Cerrar detalle del jugador">
            &times;
          </button>
        </div>
...
function openMatchDetail(matchId) {
  const match = getMatchById(appStore.tournament, matchId);
  ...
  if (appStore.ui.activePublicView === "history" && appStore.ui.activeHistoryEditionId) {
    navigateToPublicPath(`${getHistoryRouteBase(appStore.ui.activeHistoryEditionId)}/partido/${getMatchRouteId(match)}`);
    return;
  }

  navigateToPublicPath(`/draws/partido/${getMatchRouteId(match)}`);
}
...
function closeMatchDetail() {
  ...
  if (appStore.ui.activePublicView === "history" && appStore.ui.activeHistoryEditionId) {
    navigateToPublicPath(getHistoryRouteBase(appStore.ui.activeHistoryEditionId), { replace: true });
    return;
  }

  navigateToPublicPath("/draws", { replace: true });
}

function openPlayerDetail(playerId) {
  const player = appStore.tournament.lookup.playerById[playerId];

  if (!player) {
    return;
  }

  navigateToPublicPath(`/jugadores/${getPlayerRouteSlug(player)}`);
}
...
function closePlayerDetail() {
  ...
  navigateToPublicPath("/jugadores", { replace: true });
}
...
function setActivePublicView(view) {
  ...
  if (view === "players") {
    navigateToPublicPath("/jugadores");
    return;
  }

  if (view === "history") {
    navigateToPublicPath("/historia");
    return;
  }

  navigateToPublicPath("/draws");
}
...
function openHistoryEdition(editionId) {
  ...
  navigateToPublicPath(`/historia/${getEditionRouteSlug(edition)}`);
}
...
function closeHistoryEdition() {
  ...
  navigateToPublicPath("/historia", { replace: true });
}
...
function handleBrandClick(event) {
  ...
  const brandTrigger = target.closest(".app-brand");

  if (!(brandTrigger instanceof HTMLElement)) {
    return;
  }
...
function handlePopState() {
  syncPublicRouteFromLocation();
}

// App bootstrap
function initializeApp() {
  const navigation = document.querySelector(".top-nav");
  const brandTrigger = document.querySelector(".app-brand");
  ...
  if (brandTrigger) {
    brandTrigger.addEventListener("click", handleBrandClick);
  }
